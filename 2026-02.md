...
- Solargraph の導入
- Solargraph とは
	- Ruby（およびRails）コードを静的解析して、補完・定義ジャンプ・エラーチェックなどを提供するRuby用のLanguage Server
	- RubyやRailsのコードを書いているときに、メソッド補完・定義ジャンプ・簡単なエラーチェックなどが使えるようになります。
	- 具体例
		- ホバー時にメソッドのYARDコメントが表示される
			- [![Image from Gyazo](https://i.gyazo.com/3068e1ff514336eb56f65e2971fe2b77.gif)](https://gyazo.com/3068e1ff514336eb56f65e2971fe2b77)
		- Ctrlキーを押しながらクリックで定義ジャンプ
			- [![Image from Gyazo](https://i.gyazo.com/77aa350f82d01192308d2a87b8e9e0ba.gif)](https://gyazo.com/77aa350f82d01192308d2a87b8e9e0ba)
- 導入の概要
	- 環境
		- Windows11
		- WSL (Ubuntu)
		...
		- シェル:zsh
	- 概要
		- WSL上に Ruby 実行環境（rbenv + Ruby）を構築
		- WSL環境に Solargraph をインストール
		- プロジェクトごとに設定ファイル（.solargraph.yml）を作成
		- VSCode側に Solargraph 拡張機能を導入し、WSL上のSolargraphを参照させる
...
- 実施した導入方法
	- Rubyのインストール	
	    ...
	    ```bash
	    sudo apt-get update && sudo apt-get install -y autoconf patch build-essential rustc libssl-dev libyaml-dev libreadline6-dev zlib1g-dev libgmp-dev libncurses5-dev libffi-dev libgdbm6 libgdbm-dev libdb-dev uuid-dev
	    ```
	...
	- Solargraph gem のインストール
	    
	    ```bash
	    gem install solargraph
	    gem install solargraph-rails --no-document
	    ```
	    
	    solargraph-railsは、Rails 固有のメソッド（`has_many`, `belongs_to`, `scope` など）を Solargraph に認識させるプラグイン
	    
	    `--no-document`でri ドキュメント(オフラインでもRubyドキュメントを確認できるriコマンドの用のドキュメント)の生成を省略できる。
	    
	- .solargraph.ymlの作成(プロジェクトのホームディレクトリに作成)
	...
	- VSCodeの拡張機能 `Ruby Solargraph` をインストール
	- VSCode再読み込み
	- プロジェクトで使用するgemをインストール（bundle install）
	    - cdでプロジェクトのルートディレクトリに移動
	    
	    ```bash
	    bundle install
	    ```
	    
	- プロジェクトのルートディレクトリに.vscode/settings.jsonを作成(VScodeの設定→ワークスペース→設定(JSON)を開くでも可)
	    - 以下を追記
	    
	    ```bash
	    "solargraph.commandPath": "/home/ユーザー名/.rbenv/shims/solargraph"
	    ```
	    
	    ※ 実際のパスは `which solargraph` で確認する
...
### 【学んだこと】（知識・タスク分解などの振り返り）  
以前、Solargraphの導入を試みて、うまく導入できなかったことがありました。
それは、Docker上にGem solargraphをインストールしたためでした。
Solargraphは、VSCode（WSL環境）から直接参照できる場所にインストールする必要がありました。
Dockerコンテナ内にのみインストールしても、ホスト側（WSL）からは参照できないため、正しく動作しませんでした。
  
___
実務で、JavaDocコメントやeclipseのジャンプ機能をとても良く使っているので、Solargraphの導入ができ、似たようなことが出来るのがありがたいです。
ガンガン使っていこうと思います。
